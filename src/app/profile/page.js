...
function Profile() {
  ...
  const [aboutMe, setAboutMe] = useState(session?.user?.aboutMe || "");
  const [tempAboutMe, setTempAboutMe] = useState(session?.user?.aboutMe || "");
    ...
    <section className="text-base sm:text-lg text-gray-300 space-y-2 sm:space-y-4">
      <div className="flex items-center">
        ...
        {!isEditing && (
          <button
            onClick={handleEdit}
            className="ml-4 text-sm sm:text-base text-blue-500 hover:text-blue-700"
          >
            Edit
          </button>
        )}
        ...
        <section className="text-base sm:text-lg text-gray-300 space-y-2 sm:space-y-4">
          ...
          <ul className="list-disc list-inside space-y-1">
            {session.user.communitiesJoined.map(community => (
              <li key={community._id} className="text-gray-300">{community.name}</li>
            ))}
          </ul>
        </section>
